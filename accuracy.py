...
def extract_features(orb, image):
    keypoints, descriptors = orb.detectAndCompute(image, None)
    return keypoints, descriptors

def recognize_gesture(frame, gestures, orb):
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    keypoints_frame, descriptors_frame = extract_features(orb, gray_frame)
    ...
    for gesture_name, images in gestures.items():
        for template in images:
            keypoints_template, descriptors_template = extract_features(orb, template)
            
            if descriptors_frame is None or descriptors_template is None:
                continue
            
            # Match descriptors
            bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
            matches = bf.match(descriptors_frame, descriptors_template)
            
            # Compute the similarity score
            score = len(matches) / (len(keypoints_frame) + len(keypoints_template))
...
def main():
    gestures = load_gestures()
    orb = initialize_feature_detector()
    cap = cv2.VideoCapture(0)
    ...
    while True:
        ...
        recognized_gesture = recognize_gesture(frame, gestures, orb)
